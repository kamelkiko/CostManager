...
class CsvParser {
    ...
    fun parseLine(id: Int, line: String): CityEntity {
        ...
        return CityEntity(
            ...
            averageMonthlyNetSalaryAfterTax = tokens[Constants.ColumnIndex.AVERAGE_MONTHLY_NET_SALARY_AFTER_TAX].toFloatOrNull()?:0f,
            dataQuality = tokens[Constants.ColumnIndex.DATA_QUALITY].toIntOrNull() == (1 ?: 0),
            isFavourite = false,
        )
    }
    ...
    private fun constructMealsPricesFromTokens(tokenizedList: List<String>): MealsPrices {
        return MealsPrices(
            mealAtMcDonaldSOrEquivalent = tokenizedList.getFloat(Constants.ColumnIndex.MEAL_AT_MC_DONALD_S_OR_EQUIVALENT)?:0f,
            mealInexpensiveRestaurant = tokenizedList.getFloat(Constants.ColumnIndex.MEAL_INEXPENSIVE_RESTAURANT)?:0f,
            mealFor2PeopleMidRangeRestaurant = tokenizedList.getFloat(Constants.ColumnIndex.MEAL_FOR_2_PEOPLE_MID_RANGE_RESTAURANT)?:0f,
        )
    }

    private fun constructDrinksPricesFromTokens(tokenizedList: List<String>): DrinksPrices {
        return DrinksPrices(
            cappuccinoRegularInRestaurants = tokenizedList.getFloat(Constants.ColumnIndex.CAPPUCCINO_REGULAR_IN_RESTAURANTS)?:0f,
            cokePepsiAThirdOfLiterBottleInRestaurants = tokenizedList.getFloat(Constants.ColumnIndex.COKE_PEPSI_A_THIRD_OF_LITER_BOTTLE_IN_RESTAURANTS)?:0f,
            milkRegularOneLiter = tokenizedList.getFloat(Constants.ColumnIndex.MILK_REGULAR_ONE_LITER)?:0f,
            waterAThirdOfLiterBottleInRestaurants = tokenizedList.getFloat(Constants.ColumnIndex.WATER_A_THIRD_OF_LITER_BOTTLE_IN_RESTAURANTS)?:0f,
            waterOneAndHalfLiterBottleAtTheMarket = tokenizedList.getFloat(Constants.ColumnIndex.WATER_ONE_AND_HALF_LITER_BOTTLE_AT_THE_MARKET)?:0f,
        )
    }
    ...
    private fun constructFruitAndVegetablesPricesFromTokens(tokenizedList: List<String>): FruitAndVegetablesPrices {
        return FruitAndVegetablesPrices(
            apples1kg = tokenizedList.getFloat(Constants.ColumnIndex.APPLES_1KG)?:0f,
            banana1kg = tokenizedList.getFloat(Constants.ColumnIndex.BANANA_1KG)?:0f,
            oranges1kg = tokenizedList.getFloat(Constants.ColumnIndex.ORANGES_1KG)?:0f,
            tomato1kg = tokenizedList.getFloat(Constants.ColumnIndex.TOMATO_1KG)?:0f,
            onion1kg = tokenizedList.getFloat(Constants.ColumnIndex.ONION_1KG)?:0f,
            potato1kg = tokenizedList.getFloat(Constants.ColumnIndex.POTATO_1KG)?:0f,
            lettuceOneHead = tokenizedList.getFloat(Constants.ColumnIndex.LETTUCE_ONE_HEAD)?:0f
        )
    }

    private fun constructFoodPricesFromTokens(tokenizedList: List<String>): FoodPrices {
        return FoodPrices(
            loafOfFreshWhiteBread500g = tokenizedList.getFloat(Constants.ColumnIndex.LOAF_OF_FRESH_WHITE_BREAD_500G)?:0f,
            riceWhite1kg = tokenizedList.getFloat(Constants.ColumnIndex.RICE_WHITE_1KG)?:0f,
            eggsRegular12 = tokenizedList.getFloat(Constants.ColumnIndex.EGGS_REGULAR_12)?:0f,
            localCheese1kg = tokenizedList.getFloat(Constants.ColumnIndex.LOCAL_CHEESE_1KG)?:0f,
            chickenFillets1kg = tokenizedList.getFloat(Constants.ColumnIndex.CHICKEN_FILLETS_1KG)?:0f,
            beefRound1kgOrEquivalentBackLegRedMeat = tokenizedList.getFloat(Constants.ColumnIndex.BEEF_ROUND_1KG_OR_EQUIVALENT_BACK_LEG_RED_MEAT)?:0f,
        )
    }
    ...
    private fun constructServicesPricesFromTokens(tokenizedList: List<String>): ServicesPrices {
        return ServicesPrices(
            ...
            cinemaInternationalReleaseOneSeat = tokenizedList.getFloat(Constants.ColumnIndex.CINEMA_INTERNATIONAL_RELEASE_ONE_SEAT)?:0f,
            internet60MbpsOrMoreUnlimitedDataCableAdsl = tokenizedList.getFloat(Constants.ColumnIndex.INTERNET_60_MBPS_OR_MORE_UNLIMITED_DATA_CABLE_ADSL)?:0f,
            fitnessClubMonthlyFeeForOneAdult = tokenizedList.getFloat(Constants.ColumnIndex.FITNESS_CLUB_MONTHLY_FEE_FOR_ONE_ADULT)?:0f,
            tennisCourtRentOneHourOnWeekend = tokenizedList.getFloat(Constants.ColumnIndex.TENNIS_COURT_RENT_ONE_HOUR_ON_WEEKEND)?:0f,
            oneMinOfPrepaidMobileTariffLocalNoDiscountsOrPlans = tokenizedList.getFloat(Constants.ColumnIndex.ONE_MIN_OF_PREPAID_MOBILE_TARIFF_LOCAL_NO_DISCOUNTS_OR_PLANS)?:0f,
            internationalPrimarySchoolYearlyForOneChild = tokenizedList.getFloat(Constants.ColumnIndex.INTERNATIONAL_PRIMARY_SCHOOL_YEARLY_FOR_ONE_CHILD)?:0f,
    ...
    private fun constructClothesPricesFromTokens(tokenizedList: List<String>): ClothesPrices {
        return ClothesPrices(
            onePairOfJeansLevis50oneOrSimilar = tokenizedList.getFloat(Constants.ColumnIndex.ONE_PAIR_OF_JEANS_LEVIS_50ONE_OR_SIMILAR)?:0f,
            oneSummerDressInAChainStoreZaraHAndM = tokenizedList.getFloat(Constants.ColumnIndex.ONE_SUMMER_DRESS_IN_A_CHAIN_STORE_ZARA_H_AND_M)?:0f,
            onePairOfNikeRunningShoesMidRange = tokenizedList.getFloat(Constants.ColumnIndex.ONE_PAIR_OF_NIKE_RUNNING_SHOES_MID_RANGE)?:0f,
            onePairOfMenLeatherBusinessShoes = tokenizedList.getFloat(Constants.ColumnIndex.ONE_PAIR_OF_MEN_LEATHER_BUSINESS_SHOES)?:0f,
        )
    }
    ...
    private fun constructTransportationsPricesFromTokens(tokenizedList: List<String>): TransportationsPrices {
        return TransportationsPrices(
            gasolineOneLiter = tokenizedList.getFloat(Constants.ColumnIndex.GASOLINE_ONE_LITER)?:0f,
            taxi1hourWaitingNormalTariff = tokenizedList.getFloat(Constants.ColumnIndex.TAXI_1HOUR_WAITING_NORMAL_TARIFF)?:0f,
            taxi1kmNormalTariff = tokenizedList.getFloat(Constants.ColumnIndex.TAXI_1KM_NORMAL_TARIFF)?:0f,
            monthlyPassRegularPrice = tokenizedList.getFloat(Constants.ColumnIndex.MONTHLY_PASS_REGULAR_PRICE)?:0f,
            oneWayTicketLocalTransport = tokenizedList.getFloat(Constants.ColumnIndex.ONE_WAY_TICKET_LOCAL_TRANSPORT)?:0f,
            taxiStartNormalTariff = tokenizedList.getFloat(Constants.ColumnIndex.TAXI_START_NORMAL_TARIFF)?:0f
        )
    }

    private fun constructCarsPricesFromTokens(tokenizedList: List<String>): CarsPrices {
        return CarsPrices(
            volkswagenGolf_1_4_90kwTrendLineOrEquivalentNewCar = tokenizedList.getFloat(Constants.ColumnIndex.VOLKSWAGEN_GOLF_1_4_90KW_TREND_LINE_OR_EQUIVALENT_NEW_CAR)?:0f,
            toyotaCorollaSedan_1_6l_97kwComfortOrEquivalentNewCar = tokenizedList.getFloat(Constants.ColumnIndex.TOYOTA_COROLLA_SEDAN_1_6L_97KW_COMFORT_OR_EQUIVALENT_NEW_CAR)?:0f,
        )
    }

    private fun constructRealStatesPricesFromTokens(tokenizedList: List<String>): RealEstatesPrices {
        return RealEstatesPrices(
            apartmentOneBedroomInCityCentre = tokenizedList.getFloat(Constants.ColumnIndex.APARTMENT_ONE_BEDROOM_IN_CITY_CENTRE)?:0f,
            apartmentOneBedroomOutsideOfCentre = tokenizedList.getFloat(Constants.ColumnIndex.APARTMENT_ONE_BEDROOM_OUTSIDE_OF_CENTRE)?:0f,
            apartment3BedroomsInCityCentre = tokenizedList.getFloat(Constants.ColumnIndex.APARTMENT_3_BEDROOMS_IN_CITY_CENTRE)?:0f,
            apartment3BedroomsOutsideOfCentre = tokenizedList.getFloat(Constants.ColumnIndex.APARTMENT_3_BEDROOMS_OUTSIDE_OF_CENTRE)?:0f,
            pricePerSquareMeterToBuyApartmentInCityCentre = tokenizedList.getFloat(Constants.ColumnIndex.PRICE_PER_SQUARE_METER_TO_BUY_APARTMENT_IN_CITY_CENTRE)?:0f,
            pricePerSquareMeterToBuyApartmentOutsideOfCentre = tokenizedList.getFloat(Constants.ColumnIndex.PRICE_PER_SQUARE_METER_TO_BUY_APARTMENT_OUTSIDE_OF_CENTRE)?:0f,
        )
    }
